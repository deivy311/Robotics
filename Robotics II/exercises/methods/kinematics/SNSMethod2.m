...
while limit_exceeded
    ...
    qdotstar=qN_Satured+pinv(J*W)*(xdot-J*qN_Satured);%antes de entrar en esta función debe ser inicialzada con qdotstar=pinv(J)*xdot
        [maxvalue, maxindex] = max(abs(qdotstar)-abs(Qdotdmax));%q_dMaxTemp_)); % The positive values are violating the limit. We take the highest one. coloqué Qdotmax pero no sé sie stá bien
    ...
    if (Violatedvalue ) % if there is an exceeding value
        ...
        if(rank(J*W)<rank(xdot))
            ...
            limit_exceeded=false;
            disp("the task couldn be completed check rank of JW or m tal vez lo calculé mal ajaj")
        end
    end
