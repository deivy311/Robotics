function [Msubs, dynamicParamsReturn_, a] = getDynamicParametersresque(M,q, exceptionTerms)
...
Msubs = M;
aa = sym('aa', [1,20],'real'); % 20 is a random approximation. Depends on M. For RR Robot is around 5
bbD = sym('bbD', [1,20],'real'); % 20 is a random approximation. Depends on M. For RR Robot is around 5
bbDvar=bbD;
posbbD=0;
dynamicParams = [];
...
%                 end
    ...
    if iter == 2
        ...
        for i =1:length(indepSubs)
            ...
            nparams=nparams+length(indepTerm);
...
%         Msubs = subs(Msubs, aa_replace, at);
                ...
                for iba=1:size(lparamd,2)
                    ...
                    if(size(lfactmp,2)>1)
                        ...
                        for fqtemp2= lfactmp
                            ...
                            if(exTeleng>0)
                                On_exceptionTerms=  ~has(fqtemp2,exceptionTerms);
                            end
        ...
        for isgnD=1:length(dynx)
            ...
            a(isgnD)=tempsignD*a(isgnD);
        end

    
       
        Msubs = subs(Msubs, at, a(dynz));
        a = sym('a',[1, length(dynx)],'real');
